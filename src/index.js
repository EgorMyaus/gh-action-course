...
class App extends React.Component {
  ...
  loadContacts = async () => {
    try {
      if (USE_LOCAL_DATA) {
        const localContacts = JSON.parse(JSON.stringify(contactsJson)).map(contact => {
          contact.id = getNextId();
          return contact;
        });
        this.setState({ contacts: localContacts, loading: false });
      } else {
        const contacts = await contactsApi.getAll();
        const mappedContacts = contacts.map(contact => ({
          ...contact,
          id: contact.id ? contact.id.toString() : getNextId(),
        }));
        this.setState({ contacts: mappedContacts, loading: false });
      }
    } catch (err) {
      console.error('Failed to load contacts:', err);
      const fallbackContacts = JSON.parse(JSON.stringify(contactsJson)).map(contact => {
        contact.id = getNextId();
        return contact;
      });
      this.setState({ 
        ...
        loading: false,
        error: new Error('Using local data - API unavailable')
      });
    }
  };
  ...
  onCreate = async (contact, history) => {
    try {
      if (USE_LOCAL_DATA) {
        const newContact = { ...contact, id: getNextId() };
        this.setState(
          prevState => ({
            ...prevState,
        ...
        const created = await contactsApi.create(contact);
        const newContact = { ...created, id: created.id.toString() };
        this.setState(
          prevState => ({
            ...prevState,
            contacts: [...prevState.contacts, newContact],
      ...
      this.setState({ error: new Error('Failed to create contact') });
    }
  };
  ...
  onDeleteClick = async (contact, history) => {
    if (window.confirm('Are you sure you to remove this contact?')) {
      try {
        ...
        this.setState({ error: new Error('Failed to delete contact') });
      }
    }
  };


  onEdit = async (contact, history) => {
    try {
      if (!USE_LOCAL_DATA) {
      ...
      this.setState({ error: new Error('Failed to update contact') });
    }
  };
}
